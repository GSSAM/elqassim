...
const Register = ({ onToggle }) => {
  ...
  const [level, setLevel] = useState(EducationLevel.LEVEL_1);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 to-teal-100 px-4 text-right">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
        ...
        {error && <div className="bg-red-50 border-r-4 border-red-500 p-4 mb-6 text-red-700 text-sm font-medium">{error}</div>}
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <button disabled={loading} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-lg transition-colors shadow-lg shadow-emerald-200">
            {loading ? 'جاري إنشاء الحساب...' : 'تسجيل حساب'}
          </button>
        </form>

        <div className="mt-8 text-center text-sm text-gray-600">
          لديك حساب بالفعل؟ <button onClick={onToggle} className="text-emerald-600 font-bold hover:underline">سجل دخولك هنا</button>
        </div>
      </div>
    </div>
  );
